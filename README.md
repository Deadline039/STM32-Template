...
`/User/Bsp/Inc/retarget_io.h`和`/User/Bsp/Inc/uart.h`支持CMSIS Configuration Wizard，可以使用图形化界面来配置，参照：[CMSIS 头文件配置向导](https://em-ide.com/zh-cn/docs/advance/cmsis_cfg_wizard)。
...
注意：两个Target的编译参数、包含目录、编译的文件不会同步，如果修改了上述内容需要两个位置同时修改。
...
```
ADC, CAN, DAC, DMA, EXTI, FLASH, GPIO, I2C, I2S, IWDG, PWR, RTC,C SD, SPI, SRAM, TIM, UART, USART, WWDG, F(S)MC
```
